...
describe('TokenFlowCostTracker', () => {
  it('should resolve correct pricing for registered models', async () => {
    const tracker = new TokenFlowCostTracker();
    
    const claudePricing = await tracker.getPricing('claude-3-5-sonnet-20240620');
    expect(claudePricing.inputPerMillion).toBe(3.0);
    expect(claudePricing.outputPerMillion).toBe(15.0);

    const gptPricing = await tracker.getPricing('gpt-4o-mini');
    expect(gptPricing.inputPerMillion).toBe(0.15);
    expect(gptPricing.outputPerMillion).toBe(0.6);
  });

  it('should calculate cost accurately based on inputs and outputs', async () => {
    const tracker = new TokenFlowCostTracker();
    
    // GPT-4o input: 1,000,000 tokens ($5.00), output: 1,000,000 tokens ($15.00)
    // 100k input ($0.50) + 10k output ($0.15) = $0.65
    const cost = await tracker.calculateCost('gpt-4o', 100000, 10000);
    expect(cost).toBeCloseTo(0.65, 4);
  });

  it('should record transactions and check budget limits correctly', async () => {
    ...
    await tracker.recordTransaction('gpt-4o', 100000, 10000);
